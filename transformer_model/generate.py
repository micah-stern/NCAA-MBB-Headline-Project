...
python transformer_model/generate.py \
  --model_path transformer_model/models/checkpoint-3 \
  --input_text "Generate a sports headline for the game between Duke and North Carolina. Key performers: Paolo Banchero (Duke): 21 pts, 8 reb, 3 ast; Armando Bacot (North Carolina): 23 pts, 12 reb, 2 ast"

# "Generate a sports headline for the game between [TEAM1] and [TEAM2]. Key performers: [PLAYER1] ([TEAM1]): [PTS] pts, [REB] reb, [AST] ast; [PLAYER2] ([TEAM2]): [PTS] pts, [REB] reb, [AST] ast"

# Generate from input file and save to output file
python transformer_model/generate.py \
  --model_path transformer_model/models/checkpoint-3 \
  --input_file input_headlines.txt \
  --output_file generated_headlines.txt \
  --max_length 60 \
